...
{
	public class ElectronicSignScript : MonoBehaviour
	{
		...
		private void InitializeDisplay()
		{
			...
			for ( int i = 0; i < 3; i++ )
			{
				for ( int j = 0; j < 8; j++ )
				{
					...
					_panelUVs[panelNumber, 0] = FindUV( _mesh, new Vector2( panelNumber * 0.03f, 1f ) ); // upper left
					_panelUVs[panelNumber, 1] = FindUV( _mesh, new Vector2( ( panelNumber * 0.03f ) + 0.01f, 1f ) ); // upper right
					_panelUVs[panelNumber, 2] = FindUV( _mesh, new Vector2( panelNumber * 0.03f, 1f - 0.005f ) ); // lower left
					_panelUVs[panelNumber, 3] = FindUV( _mesh, new Vector2( ( panelNumber * 0.03f ) + 0.01f, 1f - 0.005f ) ); // lower right
				}
			}
